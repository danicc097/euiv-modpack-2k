...
{
	float4 vSample = GetProvinceColorSampled( uv, IndirectionMap, ProvinceIndirectionMapSize, ProvinceColorMap, ProvinceColorMapSize, 1) * 0.75f;
...
PixelShader = 
{
	...
	[[
		...
		//parameters for map look adjustment
		static const float BLEND_RATIO = 0.45f; // map look at the max zoom level
												// from 0.0f (pure terrain) to 1.0f (pure color)

		static const float DETAIL_OPACITY = 0.6f; // province detail opacity at the min zoom level
												  // from 0.5f (max detail opacity) to 1.0f (no changes from the max zoom)
									
		static const float BRIGHTNESS_MULTIPLIER = 1.25f; // general map brightness
		...
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			if ( vAllSame < 1.0f && vBorderLookup_HeightScale_UseMultisample_SeasonLerp.z < 8.0f )
			{
				...
				float4 vTestFrac = 	( vFrac.x, 1.0f - vFrac.x, vFrac.x, 1.0f - vFrac.x );
		...
		#endif
			{
				...
				float colorRange = min(testCam, 1.0f);
			
				float adjustedTerrainColorValue = max(DETAIL_OPACITY, length(vTerrainDiffuseSample.rgb));
								
				float3 vEnd = lerp(vTerrainDiffuseSample.rgb, vColorMapSample.rgb, BLEND_RATIO);
				float3 vStart = vEnd * adjustedTerrainColorValue; // multiplying by terrain diffuse color value makes dark terrain parts darker on the map
				
				vOut = lerp(vStart, vEnd, colorRange);
				vOut = CalculateMapLighting( vOut, vHeightNormalSample ) * BRIGHTNESS_MULTIPLIER; //regulates brightness of colored mapmodes on all zoom stages
